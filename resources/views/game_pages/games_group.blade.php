...
@section('content')
    ...
    <div class="padding-lr-15 col-section">
        ...
        <div class="invite-friends-to-group col-section">
            ...
            <div class="info-text-small-centered">
                <a href="{{ route('users.search') }}" class="hyperlink-text">{{ ('Uzaicināt jaunus draugus') }}</a>
            </div>
        ...
        <div class="flex-container-center pregame-continue-button">
            <button id="group-member-ready-button" class="form-submit-button {{ $is_user_ready ? 'unready' : '' }}" onclick="toggleMyReady('{{ Auth::user()->name }}')">
                ...
            </button>
        </div>

        <div class="info-text-small-centered">
            <a href="{{ route('group.leave', ['id' => $game_info->id]) }}" class="hyperlink-text">{{ ('Pamest grupu') }}</a>
        </div>
    ...
    <script type="text/javascript">
        ...
        var GAME_GROUP_START_TIME = "{{ $game_info->start_time }}";
        var GAME_GROUP_TOGGLE_USER_READY_URL = "{{ route('group.toggle_user_ready', ['id' => $game_info->id]) }}";
        var GAME_GROUP_IS_USER_READY = "{{ $is_user_ready }}";
        var GAME_GROUP_POLL_GAME_READY_URL = "{{ route('group.poll-users-ready', ['id' => $game_info->id]) }}";
        ...
        var GAME_GROUP_READY_SVG = `<x-svg.check_icon />`;
    </script>
@endsection