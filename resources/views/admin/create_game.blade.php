...
@section('content')
    ...
    <form id="admin_save_new_game" enctype="multipart/form-data" accept-charset="utf-8" action="{{ route('admin.save_new_game') }}" method="post">
        @csrf
        ...
        <div id="c_game_sec1" class="c_game_sec selected padding-lr-15 col-section">
            ...
            <input id="game_name" name="game_name" type="text" value="{{ old('game_name') }}">

            <label for="game_description">Spēles apraksts</label>
            <input id="game_description" name="game_description" type="textarea" value="{{ old('game_description') }}">

            <label for="game_start">Spēles sākums (formāts: YYYY-MM-DD HH:MM:SS)</label>
            <input id="game_start" name="game_start" type="text" value="{{ old('game_start') }}">

            <label for="game_end">Spēles sākums (formāts: YYYY-MM-DD HH:MM:SS)</label>
            <input id="game_end" name="game_end" type="text" value="{{ old('game_end') }}">
        ...
        <div id="c_game_sec2" class="c_game_sec padding-lr-15 col-section">
            ...
            <select id="existingMapSelection" name="existing_map_selection">
                <option selected="selected">- Izveidot savu karti -</option>
            ...
            <table>
                ...
                @foreach ($vietas as $vieta)
                    <tr>
                        ...
                        <td><input type="checkbox" name="vieta[]" value="{{ $vieta->id }}"></td>
                    </tr>
                @endforeach
            </table>
        ...
        <div id="c_game_sec4" class="c_game_sec padding-lr-15 col-section">
            <button id="admin_new_game_button_submit" type="submit">
                Izveidot spēli
            </button>
        </div>
